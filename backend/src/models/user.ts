...
export default {
    ...
    getById: async (id: User['id']): Promise<User | null> => {
        const user = await db.prepare('SELECT * FROM users WHERE id = ?').run(id);
        return user ? mapUser(user) : null;
    },
    create: async (data: User): Promise<User> => {
        const { email, name } = data;
        const id = randomUUID();
        const user = await createUser.run(id, email, name);
        return mapUser(user);
    },
    update: async (data: User): Promise<User> => {
        const { id, email, name } = data;
        const user = await db.prepare('UPDATE users SET email = ?, name = ? WHERE id = ? RETURNING *').run(email, name, id);
        return mapUser(user);
    },
